...
const { FrameBuffer, FrameBufferTransform } = require("../cjs");
...
function message(bytes, numLength, [read]) {
  return `should accept: ${
    bytes < Infinity ? "partial frames" : "multiple frames"
  } with ${numLength * 8}-bit ${
    read === Buffer.prototype.readUIntBE || read === Buffer.prototype.readUIntLE
      ? "unsigned"
      : "signed"
  } ${
    read === Buffer.prototype.readUIntBE || read === Buffer.prototype.readIntBE
      ? "big-endian"
      : "little-endian"
  }`;
}
